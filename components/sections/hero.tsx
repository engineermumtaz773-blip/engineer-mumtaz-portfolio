...
export function Hero() {
  ...
  return (
    <section className="relative min-h-[760px] overflow-hidden bg-navy pb-20 pt-32 text-white sm:pt-36 lg:flex lg:min-h-screen lg:items-center lg:py-28" onMouseMove={(event) => { const rect = event.currentTarget.getBoundingClientRect(); x.set((event.clientX - rect.left) / rect.width - 0.5); y.set((event.clientY - rect.top) / rect.height - 0.5); }}>
      ...
      {/* Complete Background Animation covering the full section */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0" aria-hidden="true">
        <div className="page-grid absolute inset-0 opacity-40" />
        
        {/* Full-coverage moving aurora and mesh gradients */}
        <div className="absolute -top-[50%] -left-[30%] w-[100vw] h-[100vw] rounded-full bg-gradient-to-tr from-cyan-600/25 via-indigo-600/20 to-purple-600/25 blur-[150px] animate-pulse duration-1000" />
        <div className="absolute top-[20%] -right-[30%] w-[90vw] h-[90vw] rounded-full bg-gradient-to-br from-blue-600/20 via-teal-500/15 to-cyan-400/25 blur-[160px] animate-pulse duration-700" />
        
        {/* Floating animated light blobs across the entire background */}
        <m.div 
          animate={{ x: [0, 80, -60, 0], y: [0, -60, 50, 0] }}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-1/3 left-1/4 w-[35rem] h-[35rem] rounded-full bg-cyan-500/15 blur-[130px]"
        />
        <m.div 
          animate={{ x: [0, -70, 60, 0], y: [0, 70, -40, 0] }}
          transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }}
          className="absolute bottom-1/4 right-1/4 w-[40rem] h-[40rem] rounded-full bg-indigo-500/15 blur-[140px]"
        />

        {/* Ambient glowing particles distributed across the background */}
        <div className="absolute top-16 left-[10%] w-2 h-2 rounded-full bg-cyan-300 shadow-[0_0_15px_#22d3ee] animate-ping opacity-70" style={{ animationDuration: '3.5s' }} />
        <div className="absolute top-1/3 right-[15%] w-2.5 h-2.5 rounded-full bg-blue-300 shadow-[0_0_15px_#93c5fd] animate-pulse opacity-70" style={{ animationDuration: '4s' }} />
        <div className="absolute bottom-20 left-[25%] w-2 h-2 rounded-full bg-teal-300 shadow-[0_0_15px_#5eead4] animate-ping opacity-60" style={{ animationDuration: '5s' }} />
        <div className="absolute top-2/3 right-[30%] w-2.5 h-2.5 rounded-full bg-purple-300 shadow-[0_0_15px_#d8b4fe] animate-pulse opacity-60" style={{ animationDuration: '4.5s' }} />
      </div>
      ...
      <div className="container-shell relative z-10 grid items-center gap-16 lg:grid-cols-[1.08fr_.92fr]">
        <div>
          ...
          {/* Social Icons matching the exact 3D glossy reference style with smooth hover animations */}
          <div className="mt-12 flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
                ...
                {/* Gmail */}
                <a href="https://mail.google.com/mail/?view=cm&fs=1&to=engineermumtaz773@gmail.com" target="_blank" rel="noopener noreferrer" className="group relative flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-b from-white via-slate-100 to-slate-300 text-rose-600 shadow-[0_8px_20px_rgba(244,63,94,0.4),inset_0_2px_4px_rgba(255,255,255,0.9),inset_0_-2px_4px_rgba(0,0,0,0.2)] transition-all duration-300 hover:scale-110 hover:shadow-[0_0_25px_rgba(244,63,94,0.7)]">
                  <svg className="h-6 w-6 fill-current transition-transform duration-300 group-hover:scale-110" viewBox="0 0 24 24"><path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73l-6.545 4.909-6.545-4.909v9.272H1.636C.732 21.002 0 20.27 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L12 9.928l8.073-6.435c1.618-1.214 3.927-.059 3.927 1.964z"/></svg>
                </a>
            </div>
          </div>
        </div>

        {/* Right Side: Circular Profile Image from 'mumtaz' folder with Hover crossfade */}
        <m.div className="relative mx-auto w-full max-w-[520px] perspective-[1200px] flex items-center justify-center py-10" style={{ rotateX, rotateY }}>
          ...
          >
            <div className="relative w-full h-full rounded-full overflow-hidden bg-slate-900 border-4 border-navy">
               {/* Default Profile Image inside mumtaz folder */}
               ...
               {/* Hover Profile Image inside mumtaz folder */}
               <div className={`absolute inset-0 w-full h-full transition-all duration-500 ease-in-out ${isHovered ? 'opacity-100 scale-105 rotate-2' : 'opacity-0 scale-100 rotate-0'}`}>
                 <Image src="/mumtaz/profile2.png" alt="Engineer Mumtaz Ali Hover" fill priority className="object-cover" />
               </div>
            </div>
          </m.div>
        </m.div>
